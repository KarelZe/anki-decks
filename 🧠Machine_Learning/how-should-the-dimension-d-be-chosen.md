...
marginal variance captured by \(\boldsymbol{u}_i\)</li></ul><div>
</div><div>\(\begin{aligned}
&\text { Choose } D \text { s.t. } \sum_{i=1}^{M} \lambda_{i}=\eta \underbrace{\sum_{i=1}^{D} \lambda_{i}}_{\text { Total variance of the data }}\\
\end{aligned}\)
</div>
</div>
